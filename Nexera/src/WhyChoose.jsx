...
import BackgroundImage from "/src/assets/why-bg.png";
...
const WhyChoose = () => {
  const features = [
    {
      ...
      icon: (
        <svg
          className="w-10 h-10 sm:w-12 sm:h-12 text-green-600 mb-4 flex-shrink-0"
          fill="none"
          stroke="currentColor"
          strokeWidth="2"
          viewBox="0 0 24 24"
  ...
  return (
    <section className="relative py-12 sm:py-16 bg-cover bg-center" >
      {/* Background image */}
      <div
        className="absolute left-0 -bottom-[400px] h-[900px] sm:h-[1100px] md:h-[1235px] w-full bg-no-repeat bg-center z-0"
        style={{ backgroundImage: `url(${BackgroundImage})` }}
      ></div>

      {/* Overlay (optional, if you want darken effect) */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, idx) => (
            <div
              key={idx}
              className="bg-white p-6 sm:p-8 rounded-xl shadow-md hover:shadow-lg transition duration-300 flex flex-col"
            >
              {feature.icon}
              <h3 className="text-xl font-semibold text-gray-800 mb-2">{feature.title}</h3>
              <p className="text-gray-600 mb-4 text-sm">{feature.description}</p>
              <button className="mt-auto text-green-600 font-medium text-sm hover:underline">
